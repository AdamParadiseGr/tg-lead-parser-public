...
**`reason` перед вердиктом** — chain-of-thought паттерн. Модель объясняет решение до того как его вынести. Наблюдалось снижение количества ложноположительных срабатываний по сравнению с прямым `is_lead: true/false`.
...
Каждый раз отправляется случайный вариант — Telegram не детектирует паттерн повторяющихся сообщений.
...
- **Active learning** — дообучение классификатора на накопленных ответах
